...
const UserActionsMenu = ({ row, handleOpenForm, handleOpenFormChangePassword, setOpenAlert, setClickedUser,setActionType }) => {
...
export default function ManageUsers() {
  ...
  const logoutUser = async () => {
    setLoadingUpdate(true);
    try {
      const response = await axiosInstance.post(`/users/profile/${clickedUser}/logout/`);
      if (response.status === 204) {
        handleClickVariant('تم تسجيل الخروج من الحساب بنجاح', 'success');
        getUsers();
      }
    } catch (error) {
      handleClickVariant('لقد حدث خطأ', 'error');
        ...
        loading ? <DefaultProgress sx={{ width: '100%', height: '70vh', display: 'flex' }} /> :
          <>
            ...
            <Confirm
              open={openAlert}
              onConfirm={() => {
                actionType === 'delete'?deleteUser(): actionType === 'logout'&&logoutUser();
                setOpenAlert(false);
              }}
              onClose={() => setOpenAlert(false)}
              title={actionType === 'delete'?"حذف الحساب": actionType === 'logout'&&'تسجيل خروج'}
              message={actionType === 'delete'?`هل متأكد من حذف حساب "${usersData.length > 0 ? usersData.find(user => user.profile.profile_id === clickedUser)?.profile?.full_name : 'غير معرف'}"`: actionType === 'logout'&&`تسجيل الخروج من حساب "${usersData.length > 0 ? usersData.find(user => user.profile.profile_id === clickedUser)?.profile?.full_name : 'غير معرف'}"`}
              cancelTitle="إلغاء"
              confirmTitle={actionType === 'delete'?"نعم،حذف": actionType === 'logout'&&'خروج'}
            />
            <Confirm
